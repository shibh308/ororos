...
impl OS {
    ...
    fn interrupt(&mut self) {
        ...
        let fl = self.paging.exists(5);
        write_char(fl as u8 as char);

        // TODO: 実はa0に入ってないのでは？
        let prev_page_id = set_page_id(0);
        if prev_page_id != 0 && self.paging.exists(prev_page_id) {
            self.paging.copy_from_csr(prev_page_id);
        }

        let page_id = self.paging.get_next_process(prev_page_id);
        write_char(page_id as u8 as char);

        if page_id == 0 {
            write_char(111 as u8 as char);
        ...
        /*
            TODO:
            なぜかここが動いていて, おかしい (タスクが無いのでこのまま終了するはず)
            os.pagingが全てSomeになっていて, self自体のアドレスにバグがあるかも？
        */
        write_char(110 as u8 as char);
        abort();
